...
fn main() {
    ...
    println!("   Time: {:?} (avg: {:?}/iter)", elapsed_non_fused, elapsed_non_fused / 100);
    println!("   Memory: {} KB per iteration (2 intermediate allocations)", (size * 8 * 2) / 1024);
    ...
    println!("   Time: {:?} (avg: {:?}/iter)", elapsed_fused, elapsed_fused / 100);
    println!("   Memory: {} KB per iteration (0 intermediate allocations)", 0);
    
    // Calculate speedup
    let speedup = elapsed_non_fused.as_secs_f64() / elapsed_fused.as_secs_f64();
    println!("\n⚡ SPEEDUP: {:.2}× faster", speedup);
    println!("   Memory saved: {} KB per iteration", (size * 8 * 2) / 1024);
    ...
    println!("\n=== CONCLUSION ===");
    if speedup > 1.5 {
        println!("✅ Fusion provides {:.2}× speedup - BLUEPRINT VALIDATED!", speedup);
