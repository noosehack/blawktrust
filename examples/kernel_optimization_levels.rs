...
fn main() {
    ...
    bench_kernel(
        "v0: Baseline (vec![NA])",
        || dlog_v0_baseline(&data, lag),
        iters,
    );
    bench_kernel(
        "v1: No init (MaybeUninit)",
        || dlog_v1_no_init(&data, lag),
        iters,
    );
    bench_kernel(
        "v2: No bounds checks",
        || dlog_v2_no_bounds(&data, lag),
        iters,
    );
    bench_kernel(
        "v3: No nulls fast path",
        || dlog_v3_no_nulls(&data, lag),
        iters,
    );

    let valid = vec![1u8; size];
    bench_kernel(
        "v4: Masked (with checks)",
        || dlog_v4_masked(&data, &valid, lag).0,
        iters,
    );
    bench_kernel(
        "v5: Masked fast-path",
        || dlog_v5_masked_fast(&data, None, lag).0,
        iters,
    );
    ...
    let valid_na: Vec<u8> = (0..size).map(|i| if i % 10 == 0 { 0 } else { 1 }).collect();
    ...
    bench_kernel(
        "v4: Masked",
        || dlog_v4_masked(&data_na, &valid_na, lag).0,
        iters,
    );
    bench_kernel(
        "v5: Masked (with NAs)",
        || dlog_v5_masked_fast(&data_na, Some(&valid_na), lag).0,
        iters,
    );
