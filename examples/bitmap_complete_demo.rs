...
use blawk_kdb::builtins::{dlog_column, ln_column, sentinel_to_bitmap_inplace, from_sentinel_data};
...
fn main() {
    ...
    println!("  is_all_valid: {} (fast path!)", result_clean.is_all_valid());
    ...
    let dirty_data = Column::new_f64_masked(
    ...
    let size = 1_000_000;
    let clean_large = Column::new_f64(
    ...
    println!("  Time: {:.2} ms/iter", elapsed_clean.as_millis() as f64 / iters as f64);
    
    println!("\nDirty data (masked path, 10% nulls):");
    println!("  Time: {:.2} ms/iter", elapsed_dirty.as_millis() as f64 / iters as f64);
