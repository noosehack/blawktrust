...
fn pipeline_sentinel(data: &[f64]) -> Vec<f64> {
    ...
    let step1: Vec<f64> = data.iter()
        .map(|&x| if x != NA && x > 0.0 { x.ln() } else { NA })
        .collect();
    
    let step2: Vec<f64> = step1.iter()
...
fn main() {
    ...
    let data1: Vec<f64> = (0..size)
        .map(|i| {
            if i % 100 == 0 { NA } else { 100.0 + (i as f64 * 0.01) }
        })
        .collect();
    ...
    println!("  Sentinel (3 passes, check NA each time): {:.1} ms/iter", 
        elapsed_sentinel1.as_millis() as f64 / iters as f64);
    println!("  Bitmap (1 pass, check once per 64):      {:.1} ms/iter", 
    ...
    println!("  Sentinel (still checks every element): {:.1} ms/iter", 
        elapsed_sentinel2.as_millis() as f64 / iters as f64);
    println!("  Bitmap (fast path, no branches):       {:.1} ms/iter", 
